...
function completeOrder(method) {
    ...
    // Prepare the WhatsApp Message for the Owner
    ...
    // Replace with the owner's actual number
...
// --- 6. CHATBOT LOGIC (SMART REGEX VER) ---
function toggleChat() {
...
function sendChat() {
    ...
    setTimeout(() => {
        let reply = "I am still learning! Try asking about our 'menu', 'offers', or 'events'. If you have a problem, just type 'help'.";
        
        if (msg.match(/\b(hi|hello|hey|namaste|morning|evening)\b/)) {
            reply = "Hello there! Welcome to Sample Restaurant. How can I help you today?";
        } else if (msg.match(/\b(menu|food|eat|hungry|dishes|burger|paneer|chicken)\b/)) {
            reply = "We serve delicious Veg, Non-Veg, Burgers, and Thalis! Click 'Menu' at the top of the page to explore.";
        } else if (msg.match(/\b(offer|discount|promo|cheap|price)\b/)) {
            reply = "Check our Offers page! You can use the code TODAY20 to get 20% off on orders above ₹600.";
        } else if (msg.match(/\b(problem|bad|issue|cold|wrong|complaint|help|support|stale|worst)\b/)) {
            reply = "I am so sorry to hear you are facing an issue. Please call our Support Team immediately at <strong>+91 98765 43210</strong> or message us on WhatsApp so we can fix this for you right away!";
        } else if (msg.match(/\b(where|location|address|open|time|close)\b/)) {
            reply = "We are located at 123 Food Street. We are open every day from 10:00 AM to 11:00 PM!";
        } else if (msg.match(/\b(thanks|thank you|ok|great|awesome)\b/)) {
